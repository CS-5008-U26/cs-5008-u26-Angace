...
void heapifyDown (int *a, int size, int index) {
    ...
    // which value of letf, right, and current (meaning index), is bigger
    int largest = index;

    // if current is largest, no nothing
    // if left or right is largest, swap values and repeat
    if ((left < size) && (a[left] > a[largest])) // left < size means in the size of array
    {
        int largest = left; //exchange
    }
    if ((right < size) && (a[left] > a[largest]))
    {
        largest = right;
    }
    if (largest != index) // swap
    {
        ...
        a[index] = a [largest];
        a[largest] = temp;
        heapifyDown(a, size, largest);
    }
}

void sortarray (int a[], int n) {
    // loop for half the array from the middle back to 0
    // call heapifyDown on each "node"
    for (int i=(n/2) -1; i>=0; i--)
    {
        heapifyDown(a, n, i);
    }

    // loop over the array, from back to front
    //swap root (current biggest in the heap) with the element at the back of the array
    // re-heapify
    for(int i=n-1; i>0; i--)
    {
        int  temp = a[0];
