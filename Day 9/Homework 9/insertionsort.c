/* Homework 9 - Task 3.2 insertion sort */
...
void sortarray (int a[], int n) {
    /* Insertion Sort */
    /* Insert each element into the sorted part of the array */
    for (int i = 1; i < n; i++){  //Move the current element to its correct position

        // stop if the current element is already in the correct position
        for (int j = i; j > 0; j--){
            if (a[j] >= a[j - 1]){
                break;
            }
