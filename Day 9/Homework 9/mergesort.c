/* Homework 9 - Task 3.3 merge sort */
...
void mergesort_helper(int a[], int left, int right)
{
    if (left >= right){ // base case
        return;
    }

    int mid = (left + right) / 2; // find the middle array

    mergesort_helper(a, left, mid); // sort the left half
    mergesort_helper(a, mid + 1, right); // sort he right half
