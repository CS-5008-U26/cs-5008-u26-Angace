/* Homework 9 - Task 3.1 selection sort */
...
void sortarray (int a[], int n) {
    ...
    /* Selection Sort */
    // move the largest element to the end on each pass
    for (int i = n - 1; i > 0; i--)
    {
        int largestIndex = 0; // find the largest element in the unsorted part

        // loop to find elements from largest
        for (int j = 1; j <= i; j++) {
        ...
        // exchange the large index with the temporay one
        if (largestIndex != i) {
            int temp = a[i];
            a[i] = a[largestIndex];
            a[largestIndex] = temp;
